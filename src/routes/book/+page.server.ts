    ...
    default: async ({ request }) => {
        try {
            ...
            if (!name || !email || !projectDetails) {
                return fail(400, { 
                    error: true,
                    projectDetails: 'Please fill out all required fields',
            ...
            if (!recaptchaToken) {
                return fail(400, { 
                    error: true,
                    projectDetails: 'reCAPTCHA verification is required',
                    fields: { name, email, projectDetails }
                });
            }
            ...
            if (!assessment || !assessment.passes) {
                const reason = assessment ? assessment.reason : 'Unknown reason';
                return fail(400, { 
                    error: true,
                    projectDetails: 'reCAPTCHA verification failed',
                    fields: { name, email, projectDetails },
                    reason
                });
            }
